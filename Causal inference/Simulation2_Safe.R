...
filename = 'SAFE_Simulation_results'
...
while(j <= 3 & any(class(fit.safeBCF) %in% c('error', 'NULL'))) {
  fit.safeBCF = tryCatch({
                   safeBCF_with_intervals(seed=001,
                                          y=y,
                                          original_datamat=x,
                                          ztrain=z,
...
if (class(fit.safeBCF)[1] != 'simpleError'){
  ...
  aux_safeBCF = c('safe-BCF',
                  NA,
                  NA,
                  rmse_cate(tau, tau.hat.safeBCF.test),
                  rmse_ate(tau, tau.hat.safeBCF.test))
...
if (class(fit.safeBART)[1] != 'simpleError'){
  ...
  aux_safeBART = c('safe-BCF',
                  NA,
                  NA,
                  rmse_cate(tau, tau.hat.safeBART.test),
                  rmse_ate(tau, tau.hat.safeBART.test))
  
} else {
  aux_safeBART = c('safe-BART',
                 NA,
                 NA,
                 NA,
                 NA)
