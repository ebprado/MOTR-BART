...
# Description code: A full implementation of BART in R                         #
# Last modified date: 07/07/2020                                               #
#------------------------------------------------------------------------------#

# Comments about the code:
# 1 - If you want to understand the general structure of this code, have a look only at the main function (line 120 to 225);
# 2 - The "auxiliar" functions are presented from line 250 on in the following order:
...
rBART = function(X, # X is the feature matrix 
                 y, # y is the target
                 z,
                 num_trees_mu = 10, # Number of trees (mu)
                 num_trees_tau = 10, # Number of trees (tau)
  ...
  X_orig = X # WARNING: pihat needs to be the last column
  ...
  curr_trees = create_stump(num_trees = num_trees, 
                            ...
                            X = X,
                            tau_tree = info_trees)
  # Initialise the new trees as the current ones
  new_trees = curr_trees
  
  # Initialise the predicted values to zero
  predictions = get_predictions(curr_trees, X, z, nu, single_tree = num_trees == 1)
  ...
  for (i in 1:TotIter) {
    ...
    if((i > burn) & ((i - burn) %% thin) == 0) {
      ...
      y_hat_store[curr,] = predictions_final
      tau_store[curr,] = predictions_tau
    ...
    for (j in 1:num_trees) {
      ...
      if(num_trees > 1) {
        current_partial_residuals = y_scale -
          get_predictions(curr_trees[-j], X, z, nu, single_tree = num_trees == 2)
      } else {
        current_partial_residuals = y_scale
      }
    ...
    predictions_mu = get_predictions(curr_trees[1:num_trees_mu], X, z, nu, single_tree = num_trees_mu == 1)
    predictions_tau = get_predictions(curr_trees[(num_trees_mu + 1):num_trees], X, aux_tau, nu, single_tree = num_trees_tau == 1) # Select trees of tau only
    predictions_final = predictions_mu + predictions_tau*z
    
    # Visualising the predictions at each MCMC iteration
    # Sys.sleep(0.2)
    # plot(tau, predictions_tau, main='tau'); abline(0,1)
    # Sys.sleep(0.2)
    # plot(mu, predictions_mu, main='mu'); abline(0,1)
    # Sys.sleep(0.2)
    # plot(y, predictions_final, main='final'); abline(0,1)
    
    S = sum((y_scale - predictions_final)^2)
...
create_stump = function(num_trees,
  ...
  for (j in 1:num_trees) {
    ...
    all_trees[[j]][[1]][1,] = c(1, NA, NA, NA, NA, NA, paste(rep(0, ncol(X)), collapse = ',') , length(y), tau_tree[j])
...
grow_tree = function(X, y, curr_tree, node_min_size) {
  ...
  tau_tree = as.numeric(new_tree$tree_matrix[1,'tau_tree'])
  ...
  while (bad_trees ){
    ...
    if(tau_tree == 0) {split_variable = sample(2:ncol(X), 1)}
    if(tau_tree == 1) {split_variable = sample(2:(ncol(X) - 1), 1)} # Take pihat out
    # if(tau_tree == 1) {split_variable = sample(2:ncol(X), 1)} # Keep pihat based on the bcf package documentation (https://cran.r-project.org/web/packages/bcf/bcf.pdf)
...
get_predictions = function(trees, X, zz, nu, single_tree = FALSE) {
  ...
  if(single_tree) {
    ...
    tau_tree = as.numeric(trees$tree_matrix[1, 'tau_tree'])
    ...
    if(nrow(trees$tree_matrix) == 1) {
      ...
      if (tau_tree == 0){ predictions = X%*%mu}
      if (tau_tree == 1){ predictions = (X%*%mu) * zz}
      ...
      for(i in 1:length(unique_node_indices)) {
        ...
        if (tau_tree == 0){ predictions[curr_X_node_indices == unique_node_indices[i]] = X_node%*%mu}
        if (tau_tree == 1){ predictions[curr_X_node_indices == unique_node_indices[i]] = (X_node%*%mu) * zz[curr_X_node_indices == unique_node_indices[i]]}
    ...
    predictions = get_predictions(trees[[1]], X, zz, nu, single_tree = TRUE)  + 
      get_predictions(partial_trees, X, zz, nu,
...
predict_rBART = function(newX, newZ, newPihat, rBART_posterior, 
